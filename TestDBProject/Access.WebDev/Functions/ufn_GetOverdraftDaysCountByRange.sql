CREATE FUNCTION [Access.WebDev].[ufn_GetOverdraftDaysCountByRange]
(
...
AS
-------------------------------------------------------------------------------------- 
-- Name: [Access.WebDev].[ufn_GetOverdraftDaysCountByRange]
-- 
-- Params:	@PositionDate DATE - Trade date for which the function is to be run for.
-------------------------------------------------------------------------------------- 
-- History:
-- R. Dixon: 28/11/2017 OPP-914 created
-------------------------------------------------------------------------------------- 
...
IF @PositionDate IS NULL
	...
	FROM	(
			...
			FROM
					...
					WHERE	C.CALENDARDATE BETWEEN DATEADD(D,(-1 * @DayRange),@PositionDate) AND DATEADD(D,-1,@PositionDate)
					) BV
