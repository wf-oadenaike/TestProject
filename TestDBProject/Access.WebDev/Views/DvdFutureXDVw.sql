CREATE VIEW [Access.WebDev].[DvdFutureXDVw]
	AS 
    ...
    WHERE (
	([OverrideExDate] IS NOT NULL AND [OverrideExDate] > CAST(GetDate() as date) AND [OverrideExDate] <= DATEADD(DD,-1,DATEADD(YY,DATEDIFF(YY,0,GetDate())+1,0)))
	OR ([OverrideExDate] IS NULL AND EX_DATE> CAST(GetDate() as date) AND EX_DATE <= DATEADD(DD,-1,DATEADD(YY,DATEDIFF(YY,0,GetDate())+1,0)))
	)
	AND IsActive=1;
