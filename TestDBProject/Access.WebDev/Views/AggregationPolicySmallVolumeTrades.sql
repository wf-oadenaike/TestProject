...
CREATE VIEW [Access.WebDev].[AggregationPolicySmallVolumeTrades]
/******************************
** Desc: Aggregation Policy - Utilitised by the Reporting team via EDM data generators 
		 / functions to display small volume trade violations
** Auth: R. Walker
** Date: 08/02/2018
**************************
** Change History
...
** ----			----------  -------		------------------------------------
** DAP-1745     08/02/2018  R.Walker	Initial version of view
*******************************/
AS 
...
SELECT DISTINCT 
		...
		scp.Ticker, 
		scp.OrderShares, 
		rc.ORDER_PERCENT_OF_MEDIAN_DAILY_VOLUME AS OrderPercentOfMedianDailyVolume,
		scp.[Year],
		scp.[Month],
		cmt.Concatenated AS SlackCommentary, 
		mn.MinMedianDailyVolForAggTree 
FROM CTE_Scope scp
INNER JOIN [dbo].[T_TCA_ITG_RESPONSE_ALLOCATIONS_DEAGGREGATED] rc ON scp.OrderId = rc.OrderID 
	...
	AND scp.OrderId = rc.OrderId
INNER JOIN 
	(
		SELECT	Child_Order_ID, 
				MIN(ORDER_PERCENT_OF_MEDIAN_DAILY_VOLUME) AS MinMedianDailyVolForAggTree 
		FROM [dbo].[T_TCA_ITG_RESPONSE_ALLOCATIONS_DEAGGREGATED] 
		GROUP BY Child_Order_ID
	) mn 
ON scp.Child_Order_Id = mn.Child_Order_Id -- This will find all the child orders where an actual order falls below a threshold, that way we can see all the related orders in the select statement
LEFT OUTER JOIN
