CREATE FUNCTION [Reporting.Investment].[ufn_MinimumNetCumulativeFlow12M]
(
	@StartDate DATE = NULL
	,@EndDate DATE = NULL
)
...
BEGIN
/****** Object:  UserDefinedFunction [Reporting.Investment].[ufn_MinimumNetCumulativeFlow12M]    
Script Date: 30/08/2017 08:43:00 
Author: Josef Tapper
DAP-1322
Version 1.0
******/
...
,Value
	...
	SELECT mff.[FUND_SHORT_NAME] AS [FUND_FLOW_NAME]
		  ...
		  ,SUM((case when mff.FLOW_TYPE='REDEMPTION' then -1 else 1 end) * mff.[MARKET_VALUE] * (CASE WHEN mff.CURRENCY = 'GBP' THEN 1 ELSE fx.[SPOT_RATE] END)) AS [VALUE]
   ...
   FROM
	   (
	   SELECT MIN(cumulative5) as Value
	   ,Fund
	   FROM CTE_5
	   GROUP BY 
	   Fund
   ...
   FROM
	   (
	   SELECT MIN(cumulative10) as Value
	   ,Fund
	   FROM CTE_5
	   GROUP BY 
	   Fund
   ...
   FROM
	   (
	   SELECT MIN(cumulative20) as Value
	   ,Fund
	   FROM CTE_5
	   GROUP BY 
	   Fund
