CREATE PROCEDURE [CADIS_SYS].[SPVB_DROPOBSOLETEOBJECTS]
(
...
BEGIN
/* ============================================================= */
/* ViewBuilder - Procedure to drop obsolete database objects  */
/* ============================================================= */
...
*/
PRINT ('--=================================================--')
PRINT ('--     ViewBuilder - Drop Obsolete Objects     --')
PRINT ('--  ---------------------------------------------  --')
IF UPPER(@PRINT_TO_DATATABLE) = 'Y'
BEGIN
	INSERT INTO @PrintLog ([REPORT]) VALUES (' ')
	INSERT INTO @PrintLog ([REPORT]) VALUES ('===========')
	INSERT INTO @PrintLog ([REPORT]) VALUES ('ViewBuilder')
	INSERT INTO @PrintLog ([REPORT]) VALUES ('===========')
END
...
BEGIN
	...
	IF @LOG_RUNID IS NOT NULL
		EXEC CADIS_SYS.SPLOGINFO @LOG_RUNID, 'ViewBuilder - Drop Obsolete Objects'
END
PRINT ('--=================================================--')
--
-- VIEWS
...
FOR
	SELECT DISTINCT usr.name, obj.name
		...
		INNER JOIN sys.views obj 
		   ...
		   AND obj.name LIKE 'VW[_]%')
		INNER JOIN CADIS_SYS.CO_PROCESS vb 
		ON obj.name = 'VW_' + REPLACE(vb.name,' ','_')
		WHERE vb.OBSOLETE > 0
		AND vb.NAME NOT IN (SELECT NAME FROM CADIS_SYS.CO_PROCESS WHERE OBSOLETE = 0 AND PROCESSTYPE = 25)
OPEN drop_vw_cursor;
	...
	*/
	...
	BEGIN
		...
		BEGIN
			EXEC (@SQL);
			/*****Check for errors*****/ 
			SELECT @return=@@error,@errmsg='VB: Clear Obsolete Views'; 
