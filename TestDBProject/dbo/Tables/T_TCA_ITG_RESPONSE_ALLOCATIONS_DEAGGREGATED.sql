CREATE TABLE [dbo].[T_TCA_ITG_RESPONSE_ALLOCATIONS_DEAGGREGATED] (
...
GO
CREATE NONCLUSTERED INDEX [IDX_T_TCA_ITG_RESPONSE_ALLOCATIONS_DEAGGREGATED_OrderId]
    ON [dbo].[T_TCA_ITG_RESPONSE_ALLOCATIONS_DEAGGREGATED]([OrderId] ASC)
    INCLUDE([Child_Order_Id]) WITH (FILLFACTOR = 80);


GO
CREATE NONCLUSTERED INDEX [IDX_AggregationPolicy]
    ON [dbo].[T_TCA_ITG_RESPONSE_ALLOCATIONS_DEAGGREGATED]([OrderId] ASC, [Child_Order_Id] ASC, [Decision_Date_Time] ASC)
    INCLUDE([ORDERSHARES], [SECURITY_NAME], [TICKER], [BROKER], [ACCOUNT], [REASONCODE], [ORDER_PERCENT_OF_MEDIAN_DAILY_VOLUME], [TRADE_SHARES], [TOTAL_VALUE]) WITH (FILLFACTOR = 80);
