...
CREATE FUNCTION [dbo].[UFN_UNQUOTED_PROJECTION_WIMEIF] 
(
...
AS
-------------------------------------------------------------------------------------- 
-- Name: [dbo].[UFN_UNQUOTED_PROJECTION_WIMEIF] 
-- 
...
SELECT	POSITION_DATE, FUND_SHORT_NAME, AUM, TOTAL_UNQUOTED, FLOW, FUNDING, REVALUATION, EOD, UNQUOTED_EOD 
FROM	[DBO].[UFN_UNQUOTED_PROJECTION_BASE]('WIMEIF', @DAYSBCK, @FLOWLAG)

DECLARE	@MAX_POS_DATE DATETIME
DECLARE @POSITION_DATE DATETIME
DECLARE	@PREV_AUM_EOD DECIMAL(20,2)
...
SET @MAX_POS_DATE = 
		...
		SET	@PREV_AUM_EOD = ISNULL((SELECT	TOP 1 AUM_EOD
									FROM	@OUTPUT
...
SET @LATEST_POS_DATE = (SELECT MAX(POSITION_DATE) FROM T_MASTER_POS WHERE FUND_SHORT_NAME = 'WIMEIF')
...
BEGIN 
		...
		SET @FUNDING_VAL = ISNULL((SELECT	 SUM(AllocationAmount) AS FUNDING
		...
		SET @REVAL_VAL = ISNULL((SELECT	 SUM(AllocationAmount) FUNDING
									FROM		 dbo.vw_UnquotedFundingRevaluation A
							WHERE		FUND_SHORT_NAME = 'WIMEIF'
