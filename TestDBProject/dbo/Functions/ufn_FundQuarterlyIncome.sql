...
CREATE FUNCTION [dbo].[ufn_FundQuarterlyIncome](@ValuationDate DATE)
RETURNS TABLE
AS
RETURN
(
	...
	SELECT
		...
		[EARNED_INCOME] + [TO_PAY]					AS [NET_INCOME],
		([EARNED_INCOME] + [TO_PAY]) /
	...
	FROM FundQuarterlyIncome FQI
		...
		ON FQI.[FUND_SHORT_NAME] = MFI.[FUND_SHORT_NAME]
			AND CONVERT(DATE,MFI.[VALUATION_DATE]) = @ValuationDate
			--AND DATEPART(YEAR,MFI.[VALUATION_DATE]) = 2019 --TRY_CONVERT(DATE,@ValuationDate,120)
)
