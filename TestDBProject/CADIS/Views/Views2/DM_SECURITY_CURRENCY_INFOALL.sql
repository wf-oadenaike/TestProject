CREATE VIEW "CADIS"."DM_SECURITY_CURRENCY_INFOALL" AS 
SELECT
...
CASE WHEN (RESULT.COMMENT IS NULL AND RULES.RULETYPE = 'DefaultInsert' AND MP.CADISCHANGEDBY='DataMatcherIdGenerator') THEN 'Created by Id Generator'
    ...
    FROM "CADIS_PROC"."DM_MP2_SOURCE21" MP 
        LEFT OUTER JOIN "dbo"."T_NT_CURRENCY_CODES" SR	
           ON MP."CCY_CODE" = SR."CCY_CODE"
        LEFT OUTER JOIN ( 
                    SELECT	"CCY_CODE", max(REVISION) as REVISION
                    FROM "CADIS_PROC"."DM_MP2_SOURCE21_REVISION" 
                    GROUP BY "CCY_CODE" 
                	) RV
           ON SR."CCY_CODE" = RV."CCY_CODE"
        INNER JOIN "CADIS_PROC"."DM_MP2_RESULT" RESULT 
           ON (RESULT.SOURCEID = 21 
