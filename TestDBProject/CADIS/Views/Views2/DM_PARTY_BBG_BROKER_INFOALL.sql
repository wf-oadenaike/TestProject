CREATE VIEW "CADIS"."DM_PARTY_BBG_BROKER_INFOALL" AS 
SELECT
...
CASE WHEN (RESULT.COMMENT IS NULL AND RULES.RULETYPE = 'DefaultInsert' AND MP.CADISCHANGEDBY='DataMatcherIdGenerator') THEN 'Created by Id Generator'
    ...
    FROM "CADIS_PROC"."DM_MP1_SOURCE5" MP 
        LEFT OUTER JOIN "dbo"."T_BBG_BROKER" SR	
           ON MP."BROKER" = SR."BROKER"
        LEFT OUTER JOIN ( 
                    SELECT	"BROKER", max(REVISION) as REVISION
                    FROM "CADIS_PROC"."DM_MP1_SOURCE5_REVISION" 
                    GROUP BY "BROKER" 
                	) RV
           ON SR."BROKER" = RV."BROKER"
        INNER JOIN "CADIS_PROC"."DM_MP1_RESULT" RESULT 
           ON (RESULT.SOURCEID = 5 
