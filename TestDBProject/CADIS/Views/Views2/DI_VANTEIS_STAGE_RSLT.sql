CREATE VIEW "CADIS"."DI_VANTEIS_STAGE_RSLT" AS
SELECT
...
FROM 
"CADIS_PROC"."DI_VANTEIS_RESULTS" RESULT
JOIN
(SELECT 
	ROW_NUMBER() OVER (PARTITION BY [FUND_SHORT_NAME],[EDM_SEC_ID],
		[VALUATION_DATE] ORDER BY [VALUATION_DATE] DESC)		AS [PK_RANK],
	'FUND_SHORT_NAME,EDM_SEC_ID,VALUATION_DATE'				AS [PK_COLUMNS],
	 CAST([FUND_SHORT_NAME] AS VARCHAR(50)) + ' ' + 
		CAST([EDM_SEC_ID] AS VARCHAR(10)) + ' ' +  
		CAST([VALUATION_DATE] AS VARCHAR(20))				AS [PK_VALUE],
	'Validate NT Earned Income Security'					AS [SOURCE_COMPONENT],
	* 
