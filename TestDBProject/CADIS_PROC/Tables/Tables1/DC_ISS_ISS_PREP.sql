CREATE TABLE [CADIS_PROC].[DC_ISS_ISS_PREP] (
    [EDM_ISSUER_ID]                   INT           NOT NULL,
    [ID_BB_ULTIMATE_PARENT_CO_NAME]   VARCHAR (250) NOT NULL,
    [ULT_PARENT_TICKER_EXCHANGE]      VARCHAR (250) NULL,
    [ULT_PARENT_CNTRY_OF_RISK]        VARCHAR (250) NULL,
    [ULT_PARENT_CNTRY_DOMICILE]       VARCHAR (250) NULL,
    [LEI_ULTIMATE_PARENT_COMPANY]     VARCHAR (250) NULL,
    [ID_BB_GLOBAL_ULTIMATE_PARENT_CO] VARCHAR (250) NULL,
    [ULT_PARENT_CNTRY_INCORPORATION]  VARCHAR (250) NULL,
    [COMPANYTYPE]                     VARCHAR (250) NULL,
    [SALESFORCE_ACCOUNT_ID]           VARCHAR (250) NULL,
    [BOXFOLDERID]                     VARCHAR (250) NULL,
    [FUNDMANAGERPERSONID]             SMALLINT      NULL,
    [INVESTMENTANALYSTPERSONID]       SMALLINT      NULL,
    [JIRAEPICKEY]                     VARCHAR (250) NULL,
    [FILE_NAME]                       VARCHAR (250) NULL,
    [FILE_DATE]                       VARCHAR (250) NULL,
    [CADIS_SYSTEM_INSERTED]           DATETIME      NULL,
    [CADIS_SYSTEM_UPDATED]            DATETIME      NULL,
    [CADIS_SYSTEM_CHANGEDBY]          NVARCHAR (50) NULL,
    [COMPANY_NUMBER]                  VARCHAR (250) NULL,
    [IsActive]                        BIT           NULL,
    PRIMARY KEY CLUSTERED ([ID_BB_ULTIMATE_PARENT_CO_NAME] ASC) WITH (FILLFACTOR = 80)
);

