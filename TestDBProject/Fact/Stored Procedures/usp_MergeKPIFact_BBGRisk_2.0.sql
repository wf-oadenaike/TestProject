CREATE PROCEDURE [Fact].[usp_MergeKPIFact_BBGRisk_2.0]
	@KPINAME VARCHAR(100)
AS
...
Begin Try
	...
	Declare	@strProcedureName		VARCHAR(100)	= '[Fact].[usp_MergeKPIFact_BBGRisk_2.0]';	
...
--- Performance since inception ---
	...
	,kpiname
	  FROM [Staging.BBG].[STG_Attribution] a 
...
,CASE WHEN SUBSTRING(a.Fund,1,3) = 'SJP' THEN CASE WHEN 
	...
	,kpiname
	  ...
	  LEFT JOIN
	  (SELECT * FROM [Staging.BBG].[STG_Attribution] WHERE [Fund] = 'WIMEIF') c ON 
...
UNION ALL
	...
	,convert(varchar(8),getdate(),112) + REPLACE(convert(varchar(8),getdate(),114),':','')
	,kpiname
	  FROM [Staging.BBG].[STG_Performance] a 
...
UNION ALL
	...
	,convert(varchar(8),getdate(),112) + REPLACE(convert(varchar(8),getdate(),114),':','')
	,kpiname
	  FROM [Staging.BBG].[STG_Characteristics] a 
...
UNION ALL
	...
	,convert(varchar(8),getdate(),112) + REPLACE(convert(varchar(8),getdate(),114),':','')
	,kpiname
	  FROM [Staging.BBG].[STG_Liquidity] a 
...
UNION ALL
	...
	AND Src.kpiname=@kpiname)
	WHEN NOT MATCHED and Src.KPIname=@kpiname
