...
CREATE FUNCTION [ETL].[ufn_GrossProfitMargin] 
(
)
RETURNS 
@KPIData TABLE 
...
AS
-------------------------------------------------------------------------------------- 
-- Name: [ETL].[ufn_GrossProfitMargin]
-- 
-- Params:	
-------------------------------------------------------------------------------------- 
-- History:
-- R. Dixon: 16/04/2018 DAP-1883 - Created
-------------------------------------------------------------------------------------- 
BEGIN
	DECLARE @t1 TABLE 
		(
...
; WITH DiscretionaryForecastsFix_CTE AS
	(
	...
	LEFT	JOIN [Fact].[FinancialsFact] n
...
--Load up the first set of sums
	...
	INSERT INTO @t1
	SELECT	FORMAT(LastDayOfMonth,'yyyyMMdd') AS [KPIDataDate]
	...
	FROM	(
			SELECT 
					...
					,(CASE 
						WHEN f.[NominalCode] = 7001 AND flt.[FinancialLineCode] = 'A' AND DetailNotes LIKE '%Payroll%' AND Month(c.CalendarDate) IN (6,7,10,1) THEN f.ActualAmount
					...
					,(CASE 
						WHEN flt.[FinancialLineCode] = 'FD' AND c.FiscalYear = 20152016 THEN COALESCE(cte.[difference], f.BudgetAmount) 
						ELSE f.BudgetAmount 
					END) AS Forecast
					,f.TransactionNo
					,anc.AccountName
			...
			LEFT	JOIN DiscretionaryForecastsFix_CTE cte 
			ON		cte.FinancialFactId = f.FinancialFactId
			WHERE	f.CurrentRow = 1
			AND		f.DeletedRow = 0 
			) vw
	INNER	JOIN Core.Calendar C 
	ON		C.CalendarDate=format(transactionDate,'yyyy-MM-dd 00:00:00')
	WHERE	financiallinecode = 'A'
	AND		nominalcode >= 4000
	AND		nominalcode <> 9001
	GROUP	BY format(LastDayOfMonth,'yyyyMMdd') ,format(LastDayOfMonth,'yyyy-MM-ddT00:00:00'),c.fiscalyear
  
	INSERT	INTO @t2 
	SELECT	* 
	FROM	@t1 
	WHERE	fiscalyear = (SELECT MAX(fiscalyear) FROM @t1)

	DELETE
	FROM	@t1 
	WHERE	fiscalyear <> (SELECT MAX(fiscalyear) FROM @t2)
 
	INSERT	INTO @KPIData
	SELECT	t1.KPIDataDate, SUM(t2.numerator) / SUM(t2.denominator) AS Actual, 
			t1.target,  --format(t1.LastUpdatedDate,'yyyy-MM-ddT00:00:00') 
			format(getdate(),'yyyy-MM-ddTHH:mm:ss')
	FROM	@t1 t1
	INNER	JOIN @t2 t2 
	ON		t1.KPIDataDate >= t2.KPIDataDate
