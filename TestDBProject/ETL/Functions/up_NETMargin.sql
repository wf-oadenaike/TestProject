...
-- Description:	Calculates a progressive percentages
-- for the fiscal year parameter
-- =============================================
CREATE FUNCTION [ETL].[up_NETMargin] 
(
...
join 
  ...
  insert into @t2 select * from @t1 where fiscalyear=(select max(fiscalyear) from @t1)

  delete from @t1 where fiscalyear<>(select max(fiscalyear) from @t2)
