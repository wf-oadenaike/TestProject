...
CREATE FUNCTION [Access.WebSite].[ufn_GetFundSummaryByFullHoldings]
(
...
---------------------------------------------------------------------------------------- 
---- Name: [Access.WebSite].[ufn_GetFundSummaryByFullHoldings]
---- 
...
INSERT INTO @Temp
select PREFERRED_NAME, sector, isnull(sum(CTR_TO_WI0001),0) as EndWeight,  AS_AT_DATE, 'WIMEIF' AS SEDOL from T_NT_FUND_PERFORMANCE_ATTRIBUTION
where  AS_AT_DATE = @Date
AND CTR_TO_WI0001 > 0.005
...
where  AS_AT_DATE = @Date
AND CTR_TO_WI0018 > 0.005
group by sector, AS_AT_DATE, SEDOL,PREFERRED_NAME
 
 

 insert into @Output
 SELECT  PREFERRED_NAME, SECTOR, SUM(EndWeight), AS_AT_DATE FROM @Temp
